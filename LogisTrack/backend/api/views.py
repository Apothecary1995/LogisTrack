...
class PayrollOverviewView(APIView):
    def get(self, request):
        ...
        gross_income = trip_income_total + manual_income_total
        total_expenses = service_total + fuel_total + manual_expense_total
        gross_balance = gross_income - total_expenses

        erp_setting = ERPSetting.objects.filter(company=company).first()
        vat_rate = erp_setting.vat_rate if erp_setting else Decimal("20.00")
        tax_deduction = gross_balance * (vat_rate / Decimal("100")) if gross_balance > 0 else Decimal("0")

        net_balance = gross_balance - tax_deduction
